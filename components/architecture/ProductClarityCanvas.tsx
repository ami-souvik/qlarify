...
export function ProductClarityCanvas() {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-8 bg-ivory/20 scrollbar-hide">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-charcoal tracking-tighter">Product Clarity Canvas</h1>
                        <p className="text-slate-500 font-medium uppercase tracking-widest text-[10px] mt-1">Foundational Product Strategy & Constraints</p>
                    </div>

                    <button
                        onClick={handleGenerateArchitecture}
                        disabled={!isThresholdReached}
                        className={`px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 transition-all ${isThresholdReached ? 'bg-charcoal text-white hover:bg-terracotta shadow-xl shadow-orange-900/20' : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200 opacity-50'}`}
                    >
                ...
                {!isThresholdReached && (
                    <div className="mb-8 p-4 bg-orange-50 border border-orange-100 rounded-xl flex items-center gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sections.map((section, idx) => (
                        <motion.div
                            ...
                            className={`bg-white rounded-xl border border-[#EEE9E2] shadow-xl shadow-orange-900/5 overflow-hidden flex flex-col ${section.title === "Product Overview" ? "lg:col-span-2" : ""}`}
                        >
                            <div className="px-4 py-2 border-b border-[#EEE9E2] flex items-center gap-3 bg-ivory/30">
